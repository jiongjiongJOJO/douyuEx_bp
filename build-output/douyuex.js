...
let countBpOriginal = 0;

let countBpCurrent = 0;

let isCountBpCD = false;
...
function updateCountBpDisplay() {
    ...
    if (countBpCurrent <= 0) statusElement.innerHTML = `当前已扣${countBpReduce}命，已加${countBpAdd}命，已团灭！`; else statusElement.innerHTML = `当前已扣${countBpReduce}命，已加${countBpAdd}命，剩余${calculation}命`;
}
...
function handleCountBpAdd() {
    ...
    saveData_CountBpSettings();
    let message;
    if (countBpCurrent <= 0) message = `当前已扣${countBpReduce}命，已团灭！`; else message = `当前已扣${countBpReduce}命！`;
    sendBarrage(message);
    if (countBpCd > 0) {
        isCountBpCD = true;
        setTimeout(() => {
...
function sendCountBpSummary() {
    ...
    let summaryText;
    if (countBpCurrent <= 0) summaryText = `当前已扣${countBpReduce}命，已加${countBpAdd}命，已团灭！（${calculation}）`; else summaryText = `当前已扣${countBpReduce}命，已加${countBpAdd}命，剩余${calculationAdd}命`;
...
function initPkg_CountBp_Set() {
    ...
    if (null !== ret) countBpReduce = Number(ret); else countBpReduce = 0;
    ...
    } else countBpOriginal = 0;
    ret = localStorage.getItem("ExSave_CountBpCurrent");
    if (null !== ret) countBpCurrent = Number(ret); else countBpCurrent = 0;
    updateCountBpDisplay();
}

if (!window.countBpInitialized) {
    window.countBpInitialized = true;
    setTimeout(initPkg_LiveTool_CountBp, 1e3);
}
...
function initPkg_LiveTool_Module() {
    ...
    initPkg_LiveTool_RedPacket_Bp();
    initPkg_LiveTool_CountBp();
...
function initPkg_LiveTool_HandleFunc() {
    new Ex_WebSocket_UnLogin(rid, ret => {
        ...
        initPkg_LiveTool_RedPacket_Bp_Handle(ret);
...
function saveData_isRedPacket() {
    ...
    if (ret) {
        let retJson = JSON.parse(ret);
        if ("rooms" in retJson) ridArr = retJson.rooms;
    }
    let index = ridArr.indexOf(rid);
    if (isRedPacketOn) {
        if (-1 === index) ridArr.push(rid);
    } else if (-1 !== index) ridArr.splice(index, 1);
    let data = {
        rooms: ridArr
    };
...
function FrameChase_insertIcon() {
    ...
    if (target && target.childNodes[4]) target.insertBefore(container, target.childNodes[4]); else target.appendChild(container);
}

function initPkg_VideoTools_FrameChase_Func() {
    const chaseBtn = document.querySelector("#ex-framechase-container .xg-icon");
    if (chaseBtn) chaseBtn.addEventListener("click", toggleFrameChase);
}
...
function toggleFrameChase() {
    isChasing = !isChasing;
    if (isChasing) startFrameChase(); else stopFrameChase();
}
...
function startFrameChase() {
    ...
    chaseInterval = setInterval(() => {
        ...
        if (diff > threshold) liveVideoNode.playbackRate = FAST_SPEED; else liveVideoNode.playbackRate = NORMAL_SPEED;
    }, 200);
}
